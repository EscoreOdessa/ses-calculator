...
var CFG = {
  ...
  COMPANY_CONTACTS: 'тел.: +380 __ ___ __ __   •   email: ___@___',
  CURRENCY_SYMBOL: '$',
...
var LABELS = {
  ...
  rate:       ['Курс'],
  reqCap:     ['Требуемая ёмкость АКБ', 'Необхідна ємність АКБ', 'Требуемая емкость АКБ'],
...
function generateProposalPDF() {
  ...
  if (isHybrid) {
    ...
    rows.push(['Необхідна ємність АКБ, кВт·год', fmt_(v('reqCap'))]);
  ...
  if (isHybrid || akbPrice) {
    rows.push(['Комплект АКБ (' + (isVat ? 'з ПДВ' : 'готівка') + '), ' + CFG.CURRENCY_SYMBOL, money_(akbPrice)]);
  }
  if (mountPrice) {
  ...
  rows.push(['', '']);
  rows.push(['Пропозиція попередня. Точна конфігурація та ціна — після', '']);
  rows.push(['технічного обстеження об’єкта.', '']);
